...
describe('ErrorHandler', () => {
  ...
  describe('addError', () => {
    ...
    it('should auto-dismiss non-auth errors after 5 seconds', () => {
      ...
      const result = dismissCall([{ id: errorId, type: 'api', message: 'API error', timestamp: Date.now() }])
  ...
  describe('addSuccess', () => {
    it('should add a success message to the state', () => {
      const successId = errorHandler.addSuccess('Successfully logged in!')

      expect(mockSetErrors).toHaveBeenCalledWith(expect.any(Function))
      expect(successId).toMatch(/^success_\d+_[a-z0-9]+$/)
      ...
      expect(result[0]).toMatchObject({
        id: expect.any(String),
        type: 'auth', // Success messages use 'auth' type in current implementation
        message: 'Successfully logged in!',
        timestamp: expect.any(Number),
      })
    })

    it('should auto-dismiss success messages after 3 seconds', () => {
      const successId = errorHandler.addSuccess('Success!')
