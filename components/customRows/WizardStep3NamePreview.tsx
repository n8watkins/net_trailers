...
export function WizardStep3NamePreview({
    ...
    useEffect(() => {
        uiLog('WizardStep3 - previewContent changed:', previewContent.length, 'items')
        if (previewContent.length > 0) {
            uiLog('WizardStep3 - calling onChange with previewContent')
    ...
    const fetchContentPreview = async (page: number = 1, append: boolean = false) => {
        ...
        try {
            ...
            params.append(
                'media_type',
                formData.mediaType === 'both' ? 'movie' : formData.mediaType
            )
                ...
                <div className="flex items-center gap-2">
                    {isEditingName ? (
                        ...
                        <>
                            ...
                            >
                                <ArrowPathIcon
                                    className={`w-5 h-5 ${isGeneratingName ? 'animate-spin' : ''}`}
                                />
                            </button>
                        </>
                    )}
            ...
            <div>
                <div className="flex items-center justify-between mb-3">
                    ...
                    {previewContent.length > 0 && (
                        ...
                        >
                            <SparklesIcon
                                className={`w-4 h-4 ${isLoadingMoreSuggestions ? 'animate-pulse' : ''}`}
                            />
                        ...
                        {previewContent.length > 10 && (
                            <div className="flex items-center justify-center gap-2 mt-4">
                                ...
                                <span className="text-sm text-gray-400">
                                    Page {currentPreviewPage + 1} of{' '}
                                    {Math.ceil(previewContent.length / 10)}
                                </span>
                                <button
                                    onClick={() =>
                                        setCurrentPreviewPage((prev) =>
                                            Math.min(
                                                Math.ceil(previewContent.length / 10) - 1,
                                                prev + 1
                                            )
                                        )
                                    }
