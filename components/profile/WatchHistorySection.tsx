/**
 * Watch History Section Component - Cinematic Edition
 *
...
function ProfileImageCard({ content }: { content: Movie | TVShow }) {
    ...
    return (
        <div className="group relative flex-shrink-0 w-28 sm:w-32 transition-all duration-300 hover:scale-105 hover:-translate-y-1">
            ...
            <div className="absolute -inset-1 bg-gradient-to-br from-purple-500/15 via-violet-500/10 to-purple-500/15 rounded-lg blur-md opacity-0 group-hover:opacity-100 transition-all duration-300" />

            {/* Poster container */}
            <div className="relative aspect-[2/3] overflow-hidden rounded-lg">
                {/* Rim light effect */}
                <div className="absolute -inset-0.5 bg-gradient-to-br from-purple-400/20 via-violet-500/15 to-purple-400/20 rounded-lg blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300" />

                {imageToUse ? (
                    <img
                        src={imageToUse}
                        alt={getTitle(content)}
                        className="relative w-full h-full object-cover rounded-lg ring-1 ring-white/10 group-hover:ring-purple-400/30 transition-all duration-300 shadow-lg group-hover:shadow-[0_8px_25px_rgba(168,85,247,0.2)]"
...
export function WatchHistorySection({
    ...
    const viewAllUrl = isPublic ? `/users/${userId}/history` : '/history'

    if (watchHistoryPreview.length === 0 && isPublic) {
        return null // Don't show empty section on public profiles
    }

    return (
        <section id="watch-history-section" className="relative group/section">
            ...
            <div className="relative bg-gradient-to-br from-purple-950/20 via-zinc-900/80 to-violet-950/20 backdrop-blur-lg rounded-xl overflow-hidden border border-purple-500/10 group-hover/section:border-purple-500/20 transition-all duration-300 shadow-lg">
                ...
                <div className="p-5 sm:p-6">
                    ...
                    <div className="flex items-center justify-between mb-5">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="relative">
                                ...
                                <ClockSolidIcon className="relative w-6 h-6 text-purple-400 drop-shadow-[0_0_8px_rgba(168,85,247,0.5)]" />
                            </div>
                    ...
                    {/* Content Row */}
                    {watchHistoryPreview.length > 0 ? (
                        <div className="relative">
                            {/* Ambient underglow for row */}
                            <div className="absolute inset-x-0 -bottom-2 h-8 bg-gradient-to-t from-purple-500/5 to-transparent blur-lg" />

                            {/* Scrollable poster row */}
                            <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                                {watchHistoryPreview.slice(0, limit).map((content, index) => (
                                    <div
                                        key={content.id}
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-center">
                            <div className="relative mb-4">
                                ...
                                <div className="relative w-20 h-20 rounded-full bg-zinc-900/60 backdrop-blur-lg flex items-center justify-center border border-zinc-800/50">
                                    <ClockIcon className="w-10 h-10 text-purple-900" />
                                </div>
                            </div>
                            <p className="text-gray-400 text-sm">No watch history</p>
                            <p className="text-gray-500 text-xs mt-1">
                                Start watching movies & shows
                            </p>
                        </div>
                    )}
                </div>
