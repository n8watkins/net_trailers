...
export default function TrendingActorsRow({ onLoadComplete }: TrendingActorsRowProps) {
    ...
    const openActorContentModal = useModalStore((state) => state.openActorContentModal)
    ...
    // Handle actor click - open modal with their content
    const handleActorClick = useCallback(
        (actor: TrendingPerson) => {
            openActorContentModal(actor)
        },
        [openActorContentModal]
    )
                ...
                >
                    {actors.map((actor) => (
                        <div key={actor.id} className="flex-shrink-0">
                            <ActorCard actor={actor} onClick={handleActorClick} />
                        </div>
                    ))}
                </div>
